...
/**
 *
 * Add a build phase, adds the files specified in the filePathsArray as new files.
 * Use the existingFiles flag to indicate that the files array are existing PBXFile references
 *
 * @param opts.filePathsArray - These files are added as PBXFile references
 * @param opts.existingFiles - If true, considers the supplied files in filePathsAray to be PBXFile reference objects
 * @param opts.buildPhaseType - 'isa' value in pbxproj file
 * @param opts.comment
 * @param opts.target - specified target uuid, defaults tofirstTarget
 * @param opts.folderType - Only used for PBXCopyFilesBuildPhase
 * @param opts.subfolderPath - Only used for PBXCopyFilesBuildPhase
 * @returns {{uuid, buildPhase: {isa: *, buildActionMask: number, files: Array, runOnlyForDeploymentPostprocessing: number}}}
 */
pbxProject.prototype.addBuildPhase = function(opts) {
    var filePathsArray = opts.filePathsArray || [],
        existingFiles = opts.existingFiles,
        buildPhaseType = opts.buildPhaseType,
    ...
    if (buildPhaseType === 'PBXShellScriptBuildPhase') {
        console.log('in shell build phase');
        buildPhase.shellPath = shellPath;
    ...
    if(existingFiles) {
        for (var index = 0; index < filePathsArray.length; index++) {
           buildPhase.files.push(filePathsArray[index])
        }
    } else {
        for (var index = 0; index < filePathsArray.length; index++) {
            var filePath = filePathsArray[index],
 ...
 * @param type
 * @param subfolder
 * @param [buildConfigurationsList]
 * @param [pbxNativeTarget]
 * @returns {{uuid, pbxNativeTarget: {isa: string, name: string, productName: string, productReference: *, productType: string, buildConfigurationList, buildPhases: Array, buildRules: Array, dependencies: Array}}}
 */
pbxProject.prototype.addTarget = function(name, type, subfolder, buildConfigurationsList, pbxNativeTarget) {
    ...
    var buildConfigurationsList = buildConfigurationsList || [
        {
            name: 'Debug',
    ...
    if (targetType === 'app_extension') {
        ...
        this.addBuildPhase({
            filesArray: [],
            buildPhaseType: 'PBXCopyFilesBuildPhase',
            comment: 'Copy files',
            target: this.getFirstTarget().uuid, folderType: targetType
        });
